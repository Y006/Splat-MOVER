...
import cv2
import torch
# Set default CUDA device to avoid memory issues
...
if gaussian_splatting:
    config_path = Path(f"/home/jatucker/data/data/splats/gsplats/right_gate_9_30_2025_COLMAP/sagesplat/2025-10-01_103533/config.yml")
else:
    # ASK-NeRF
    config_path = Path(f"<config.yml>")

# name of the scene
scene_name = "right_gate_9_30_2025_COLMAP"
...
for trial_idx in range(num_trials):
    ...
    # objects to move
    objects: List[str] = ["stuffed penguin"]
    ...
    table_pcd_sim = table_attr["raw_similarity"][table_sim_mask].float().cpu().numpy()

    # Remove spurious table points using axis-aligned bounds.
    table_x_max = 0.0744
    table_x_min = -0.0859065
    table_y_min = -0.1320343
    table_y_max = -0.01746813
    table_z_min = -0.1575694
    table_z_max = -0.07407138
    table_clean_mask = (
        (table_pcd_points[:, 0] < table_x_max)
        & (table_pcd_points[:, 0] >= table_x_min)
        & (table_pcd_points[:, 1] >= table_y_min)
        & (table_pcd_points[:, 1] <= table_y_max)
        & (table_pcd_points[:, 2] >= table_z_min)
        & (table_pcd_points[:, 2] <= table_z_max)
    )
    table_mask = table_sim_mask.copy()
    table_mask[table_sim_mask] = table_clean_mask
    table_pcd_points = table_pcd_points[table_clean_mask]
    table_pcd_colors = table_pcd_colors[table_clean_mask]
    table_pcd_sim = table_pcd_sim[table_clean_mask]
    np.save(f"{pcd_output_dir}/table_mask_right.npy", table_mask)
    np.save(f"{pcd_output_dir}/table_points_right.npy", table_pcd_points[:, :3])
    ...
    o3d.io.write_point_cloud(f"{pcd_output_dir}/table_point_cloud_right.ply", pcd_clus)
    ...
    if enable_visualization_pcd:
        ...
        # if save_pcd:
        #     o3d.io.write_point_cloud(f"{pcd_output_dir}/table_point_cloud.ply", pcd)
        # else:
    ...
    # gate location
    gate_centroid, gate_z_bounds, scene_pcd, gate_sim_mask, gate_attr = (
        get_centroid(
            nerf=nerf,
            env_pcd=env_pcd,
            pcd_attr=env_attr,
            positives="white orange blue pvc pipe not ground",
            threshold=0.95,
            enable_convex_hull=False,
            enable_spherical_filter=True,
            use_Mahalanobis_distance=False,
    ...
    # table data
    gate_pcd_points = np.asarray(scene_pcd.points)[gate_sim_mask]
    gate_pcd_colors = np.asarray(scene_pcd.colors)[gate_sim_mask]
    gate_pcd_sim = gate_attr["raw_similarity"][gate_sim_mask].float().cpu().numpy()

    gate_bottom_z_min = -0.04046468
    gate_bottom_z_max = 0.07885233
    gate_bottom_x_min = -0.116
    gate_bottom_x_max = -0.002331966
    gate_bottom_y_min = 0.01132468
    gate_bottom_y_max = 0.1499937
    gate_clean_mask = (
        (gate_pcd_points[:, 2] >= gate_bottom_z_min)
        & (gate_pcd_points[:, 2] <= gate_bottom_z_max)
        & (gate_pcd_points[:, 0] >= gate_bottom_x_min)
        & (gate_pcd_points[:, 0] <= gate_bottom_x_max)
        & (gate_pcd_points[:, 1] >= gate_bottom_y_min)
        & (gate_pcd_points[:, 1] <= gate_bottom_y_max)
    )
    gate_mask = gate_sim_mask.copy()
    gate_mask[gate_sim_mask] = gate_clean_mask
    gate_pcd_points = gate_pcd_points[gate_clean_mask]
    gate_pcd_colors = gate_pcd_colors[gate_clean_mask]
    gate_pcd_sim = gate_pcd_sim[gate_clean_mask]
    np.save(f"{pcd_output_dir}/gate_mask_right.npy", gate_mask)
    np.save(f"{pcd_output_dir}/gate_points_right.npy", gate_pcd_points[:, :3])

    # #%%
    # # # # #
    # # # # # Plane Fitting for the Gate
    # # # # #
    gate_pcd_clus = o3d.geometry.PointCloud()
    gate_pcd_clus.points = o3d.utility.Vector3dVector(gate_pcd_points[:, :3])
    gate_pcd_clus.colors = o3d.utility.Vector3dVector(gate_pcd_colors)
    
    # Calculate gate centroid and add as a pink point for visualization
    gate_centroid = np.mean(gate_pcd_points[:, :3], axis=0)
    
    # Save gate centroid to npy file
    np.save(f"{pcd_output_dir}/gate_centroid_right.npy", gate_centroid)
    
    # Add centroid point to the point cloud
    centroid_point = gate_centroid.reshape(1, 3)
    pink_color = np.array([[1.0, 0.0, 1.0]])  # Pink color (RGB)
    
    # Combine original points with centroid point
    all_points = np.vstack([gate_pcd_points[:, :3], centroid_point])
    all_colors = np.vstack([gate_pcd_colors, pink_color])
    
    # Update the point cloud with combined points and colors
    gate_pcd_clus.points = o3d.utility.Vector3dVector(all_points)
    gate_pcd_clus.colors = o3d.utility.Vector3dVector(all_colors)
    
    o3d.io.write_point_cloud(f"{pcd_output_dir}/gate_point_cloud_right.ply", gate_pcd_clus)



    # gate bottom location
    gate_bottom_centroid, gate_bottom_z_bounds, scene_pcd, gate_bottom_sim_mask, gate_bottom_attr = (
        get_centroid(
            nerf=nerf,
            env_pcd=env_pcd,
            pcd_attr=env_attr,
    ...
    # table data
    gate_bottom_pcd_points = np.asarray(scene_pcd.points)[gate_bottom_sim_mask]
    gate_bottom_pcd_colors = np.asarray(scene_pcd.colors)[gate_bottom_sim_mask]
    gate_bottom_pcd_sim = gate_bottom_attr["raw_similarity"][gate_bottom_sim_mask].float().cpu().numpy()

    # Remove spurious gate-bottom points outside desired x/z bounds.
    gate_bottom_z_min = -0.1539756
    gate_bottom_z_max = 0.07885233
    gate_bottom_x_min = -0.116
    gate_bottom_x_max = -0.002331966
    gate_bottom_y_min = 0.01132468
    gate_bottom_y_max = 0.1499937
    gate_bottom_clean_mask = (
        (gate_bottom_pcd_points[:, 2] >= gate_bottom_z_min)
        & (gate_bottom_pcd_points[:, 2] <= gate_bottom_z_max)
        & (gate_bottom_pcd_points[:, 0] >= gate_bottom_x_min)
        & (gate_bottom_pcd_points[:, 0] <= gate_bottom_x_max)
        & (gate_bottom_pcd_points[:, 1] >= gate_bottom_y_min)
        & (gate_bottom_pcd_points[:, 1] <= gate_bottom_y_max)
    )
    gate_bottom_mask = gate_bottom_sim_mask.copy()
    gate_bottom_mask[gate_bottom_sim_mask] = gate_bottom_clean_mask
    gate_bottom_pcd_points = gate_bottom_pcd_points[gate_bottom_clean_mask]
    gate_bottom_pcd_colors = gate_bottom_pcd_colors[gate_bottom_clean_mask]
    gate_bottom_pcd_sim = gate_bottom_pcd_sim[gate_bottom_clean_mask]
    np.save(f"{pcd_output_dir}/gate_bottom_mask_right.npy", gate_bottom_mask)
    np.save(f"{pcd_output_dir}/gate_bottom_points_right.npy", gate_bottom_pcd_points[:, :3])

    # #%%
    # # # # #
    # # # # # Plane Fitting for the Gate
    # # # # #
    gate_bottom_pcd_clus = o3d.geometry.PointCloud()
    gate_bottom_pcd_clus.points = o3d.utility.Vector3dVector(gate_bottom_pcd_points[:, :3])
    gate_bottom_pcd_clus.colors = o3d.utility.Vector3dVector(gate_bottom_pcd_colors)
    
    # Calculate gate centroid and add as a pink point for visualization
    # gate_centroid = np.mean(gate_pcd_points[:, :3], axis=0)
    
    # Save gate centroid to npy file
    np.save(f"{pcd_output_dir}/gate_bottom_centroid_right.npy", gate_bottom_centroid)
    
    # Add centroid point to the point cloud
    centroid_point = gate_bottom_centroid.reshape(1, 3)
    pink_color = np.array([[1.0, 0.0, 1.0]])  # Pink color (RGB)
    
    # Combine original points with centroid point
    all_points = np.vstack([gate_bottom_pcd_points[:, :3], centroid_point])
    all_colors = np.vstack([gate_bottom_pcd_colors, pink_color])
    
    # Update the point cloud with combined points and colors
    gate_bottom_pcd_clus.points = o3d.utility.Vector3dVector(all_points)
    gate_bottom_pcd_clus.colors = o3d.utility.Vector3dVector(all_colors)
    
    o3d.io.write_point_cloud(f"{pcd_output_dir}/gate_bottom_point_cloud_right.ply", gate_bottom_pcd_clus)
    ...
    # Remove spurious gate-bottom points outside desired x/z bounds.
    gate_bottom_z_min = -0.1977799
    gate_bottom_x_min = -0.039401330
    gate_bottom_y_min = 0.001
    gate_bottom_clean_mask = (
        (gate_bottom_pcd_points[:, 2] >= gate_bottom_z_min)
        & (gate_bottom_pcd_points[:, 0] >= gate_bottom_x_min)
        & (gate_bottom_pcd_points[:, 1] <= gate_bottom_y_min)
    ...
    o3d.io.write_point_cloud(f"{pcd_output_dir}/gate_bottom_point_cloud.ply", gate_bottom_pcd_clus)
    ...
    #     target_pcd_sim = target_attr["raw_similarity"][target_sim_mask].float().cpu().numpy()
    ...
    # filter-size
    filter_radius = [0.00055, 0.05, 0.05, 0.05]
    # TODO
    ...
    for idx, obj in enumerate(objects):
        ...
        src_centroid, src_z_bounds, scene_pcd, similarity_mask, other_attr = (
            ...
            env_pcd=env_pcd,
            pcd_attr=env_attr,
            positives="stuffed penguin",
            threshold=0.9,
            enable_convex_hull=False,
            enable_spherical_filter=True,
            filter_radius=0.0005,
            visualize_pcd=False,
            )
        )
        #     get_centroid(
        ...
        object_pcd_sim = other_attr["raw_similarity"][similarity_mask].float().cpu().numpy()
        ...
        if enable_visualization_pcd:
            ...
            if save_pcd:
                o3d.io.write_point_cloud(f"{pcd_output_dir}/object_point_cloud.ply", pcd_clus)
            else:
        ...
        comp_mask[comp_mask == True] = torch.tensor(new_obj_mask).to(comp_mask.device)
        ...
        object_pcd_points_upd = np.asarray(
            nerf.pipeline.model.means[comp_mask].clone().detach().float().cpu().numpy()
        )
        object_pcd_colors_upd = np.asarray(
            nerf.pipeline.model.features_dc[comp_mask].clone().detach().float().cpu().numpy()
        )
        ...
        cloud_masked = np.asarray(object_pcd_points[:, :3])
        color_masked = np.asarray(object_pcd_colors)
        ...
        import pdb; pdb.set_trace()
        # reoriented grasps
        reor_gp = reorient_grasps(gg[:num_vis_grasp], normal_plane)
        ...
        if enable_minimal_visualization:
            if save_pcd:
                fig_grasp_w_aff_w_pp.savefig(f"{pcd_output_dir}/grasp_point_cloud.png")
            else:
        ...
        save_grasps(
            filename=pose_filename, grasps=gg, num_grasps_save=num_grasps_save
        )
